...
//        flumentClient.create().forPath(path,"普通持久节点".getBytes());
//        flumentClient.create().withMode(CreateMode.EPHEMERAL_SEQUENTIAL).forPath("/temp" ,"临时节点".getBytes());
//        flumentClient.create().creatingParentContainersIfNeeded().forPath(path + "/parent/son1","根节点,父亲节点,儿子节点".getBytes());
